...
// The Fox venue ID
const VENUE_ID = 'ba9c45d4-3947-4560-9327-7f00c695d177';
const VENUE_NAME = 'The Fox';
...
async function main() {
  ...
  try {
    ...
    while (currentDate <= endDate) {
      // 40-60 responses per day (smaller venue)
      const responsesPerDay = Math.floor(Math.random() * 21) + 40;
      dayCount++;
      ...
      for (let i = 0; i < responsesPerDay; i++) {
        ...
        // Determine sentiment (60% positive, 28% neutral, 12% negative - needs improvement)
        ...
        // 75% of feedback should be resolved (needs improvement)
        ...
        if (Math.random() < 0.10) {
          ...
          const assistanceResolved = Math.random() < 0.92;
    ...
    // Starting ratings (Fox starts lower - room for improvement)
    let tripAdvisorRating = 3.5;
    let tripAdvisorCount = 65;
    let googleRating = 3.4;
    let googleCount = 98;

    // Target ratings
    const tripAdvisorTarget = 4.2;
    const googleTarget = 4.1;
    ...
    while (ratingsCurrentDate <= historicalEnd) {
      ...
      tripAdvisorCount += Math.floor(Math.random() * 2) + 1;
      googleCount += Math.floor(Math.random() * 3) + 1;
    ...
    while (npsCurrentDate <= endDate) {
      // 15-28 NPS submissions per day (smaller venue)
      const npsPerDay = Math.floor(Math.random() * 14) + 15;
