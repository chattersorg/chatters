...
// The Dunn Inn venue ID
const VENUE_ID = 'd7683570-11ac-4007-ba95-dcdb4ef6c101';
const VENUE_NAME = 'The Dunn Inn';
...
// Main execution
async function main() {
  console.log('🚀 Starting demo data insertion for The Dunn Inn...\n');
  ...
  try {
    ...
    if (empError || !employees || employees.length === 0) {
      console.error('❌ No employees found for The Dunn Inn');
      process.exit(1);
    }
    ...
    while (currentDate <= endDate) {
      // 60-80 responses per day
      const responsesPerDay = Math.floor(Math.random() * 21) + 60;
      dayCount++;
      ...
      for (let i = 0; i < responsesPerDay; i++) {
        ...
        // 85% of feedback should be resolved
        const shouldResolve = Math.random() < 0.85;
        ...
        // 15% chance of assistance request
        if (Math.random() < 0.15) {
    ...
    // Starting ratings
    let tripAdvisorRating = 3.8;
    let tripAdvisorCount = 95;
    let googleRating = 3.6;
    let googleCount = 142;

    // Target ratings (gradual improvement over 30 days)
    const tripAdvisorTarget = 4.5;
    const googleTarget = 4.3;
    ...
    while (npsCurrentDate <= endDate) {
      // 25-40 NPS submissions per day
      const npsPerDay = Math.floor(Math.random() * 16) + 25;
    ...
    console.log('\n✅ Demo data insertion complete for The Dunn Inn!\n');
    console.log('==========================================');
    console.log('Summary:');
    console.log('==========================================');
    console.log(`✓ ${allFeedback.length} feedback records (85% resolved by staff)`);
    console.log(`✓ ${allAssistance.length} assistance requests`);
    console.log(`✓ ${allHistoricalRatings.length} historical rating records`);
    console.log(`✓ TripAdvisor: 3.8 → 4.5 | Google: 3.6 → 4.3`);
